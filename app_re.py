...
# INTERFACE
# ==========================================
st.title("📊 Conciliador: Conta Corrente x Conta Contábil")
st.markdown("---")
    ...
    if not uploaded_files:
        ...
        if not pares:
            ...
            for idx, par in enumerate(pares):
                ...
                with st.container():
                    ...
                    try:
                        par['excel'].seek(0)
                        try:
                            df_raw = pd.read_csv(par['excel'], header=None, encoding='latin1', sep=',', engine='python')
                        except:
                            df_raw = pd.read_excel(par['excel'], header=None)
                        ...
                        if not idx_cabecalho.empty:
                            ...
                            # Usamos zfill(2) para garantir que '7' vire '07'
                            ...
                            logs.append(f"⚠️ UG {ug}: Cabeçalho 'Conta Corrente' não encontrado no Excel.")
                                    ...
                                    if re.match(r'^"?\d+"?\s*[-]?\s*[A-Za-z]', line) or re.match(r'^"?\d+', line):
                                        ...
                                        if len(vals) >= 1:
                                            ...
                                            if chave_match:
                                                ...
                                                # Pega os 2 últimos dígitos (ex: 33903007 -> 07)
                                                chave_vinculo = str(conta_contabil_completa[-2:]).zfill(2)
                                                
                                                # O Saldo Atual é a última coluna no PDF
            ...
            try:
                ...
                st.download_button("BAIXAR RELATÓRIO DE CONCILIAÇÃO PDF", pdf_bytes, "RELATORIO_CONTA_CORRENTE.pdf", "application/pdf", type="primary", use_container_width=True)
            except Exception as e:
